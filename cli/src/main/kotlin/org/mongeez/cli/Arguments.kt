...
internal class Arguments {
    @Parameter(names = ["-c", "--changeSetListFile", "--change-set-list-file"], description = "Path of the file that contains descriptions of changeset files", required = true, order = 0)
    var changeSetListFileParameter: String? = null
    ...
    var port = 27017
    @Parameter(names = ["-d", "--databaseName", "--database-name"], description = "Name of the database on which the change sets will be executed", order = 3)
    var databaseName = "test"
    @Parameter(names = ["-ae", "--authenticationEnabled", "--authentication-enabled"], arity = 1, description = "Option to enable/disable authentication on database", order = 4)
    var authenticationEnabled = true
    @Parameter(names = ["-ad", "--authenticationDatabase", "--authentication-database"], description = "Name of the authentication database", order = 5)
    var authenticationDatabase = "admin"
    @Parameter(names = ["-u", "--userName", "--user-name"], description = "Name of the user to authenticate to database (should not be used unless parameters are passed by properties file)", order = 6)
    ...
    @Parameter(names = ["-s", "--useMongoShell", "--use-mongo-shell"], arity = 1, description = "Option to use mongo shell or 'db.eval' to execute change sets", order = 9)
    var useMongoShell = true
    @Parameter(names = ["--dryRun", "--dry-run"], description = "Sets the option to dry run change sets (lists all change sets that would be executed and the last executed changeset)", order = 10)
    var dryRun = false
    ...
    val changeSetListFile: String
        get() = changeSetListFileParameter
                ?: throw IllegalStateException("Change set list file parameter is mandatory")
